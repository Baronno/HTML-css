...
1.  `HTML5` 的概念与定义 
   ...
   - 定义：`HTML5` 定义了 `HTML` 标准的最新版本，是对 `HTML` 的第五次重大修改，号称下一代的 `HTML` ， XHTML在做移动端开发时会比较吃力，例如旋转菱形内容。
   - 两个概念：
...
<body>
  ...
  <!-- 
   ...
   -->
   <!--control 控件属性，显示控件。autoplay 在音频就绪后马上播放 loop 循环-->
   <audio src ="./media/snow.mp3 controls="controls"></audio>
   <audio src ="./media/snow.mp3 controls="controls" autoplay= "autoplay"></audio>
...
<img src="images\newinput.png">
required 必须填 placeholder 背景词 autofoucs 自动聚焦到框 autocomplete 会显示之前输入的值mutiple 多个提交
code display 
<form action ="">
